...
try:
  ...
  cursor = con.cursor()
    ...
    if isinstance(e, py.ProgrammingError):
         print('Login to the SQL Server failed:')
         messagebox.showinfo(title='Car Database', message=str(e))
         sys.exit()

    else:
        print('An error occurred while connecting to the SQL Server:')
        messagebox.showinfo(title='Car Database', message=str(e))
        sys.exit()


except:
      messagebox.showinfo(title = 'Car Database',message= 'Database connection error')
      sys.exit()
...
class rustycarhiredb:
      ...
      def insert(self,RegNo,Make,EngineSize,DateRegistered,RentalPerDay,Available):#worked
         try:
            ...
            if len(RegNo) == 0 :
                 raise ValueError('zero-length entry is not allowed for  Car RegNO')

            elif len(Make)==0:
                raise ValueError('zero-length entry is not allowed for Car Make ')

            elif len(EngineSize)==0:
                raise ValueError('zero-length entry is not allowed for Car EngineSize ')

            elif len(DateRegistered)==0:
                raise ValueError('zero-length entry is not allowed for Car DateRegistered')

            elif len(RentalPerDay)==0:
                raise ValueError('zero-length entry is not allowed for Car RentalPerDay ')

            elif len(Available)==0:
                raise ValueError('zero-length entry is not allowed for Car Available')
            ...
            messagebox.showinfo(title = 'Car Database',message='new car has been added')
                     ...
                     messagebox.showinfo(title = 'Car Database',message='Cannot insert duplicate key in object dbo.Cars')
      ...
      def delete (self,RegNo):#worked
        ...
        try:
            if len(RegNo) == 0  :
              raise Exception('zero-length entry is not allowed for RegNo')
